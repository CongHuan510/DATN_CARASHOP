...
public class CartController extends BaseController implements Jw27Constants {
	...
	public ResponseEntity<Map<String, Object>> updateProductQuantity(final HttpServletRequest request,
			@RequestBody ProductCart productCart, final Model model) throws IOException {
		HttpSession session = request.getSession();
		Map<String, Object> jsonResult = new HashMap<String, Object>();
		if (session.getAttribute("cart") != null) {
			Cart cart = (Cart) session.getAttribute("cart");
			...
			if (index != -1) {
				...
				// jsonResult.put("message", "Thêm số lượng sản phẩm " + dbProduct.getName() + "
				// thành công!");
	            ...
	            //System.out.println("New Total Price (before formatting): " + newTotalPrice);

	            // Chuyển đổi thành chuỗi để tránh làm tròn
//	            String formattedNewTotalPrice = newTotalPrice.toPlainString();
//	            System.out.println(formattedNewTotalPrice);
	...
	public String deleteProductFromCart(@PathVariable("productId") int productId, @PathVariable("size") String size,
